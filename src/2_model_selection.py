...
def featureSet(data):
    data_num = len(data)
    new_df = data.drop("target", axis=1)
    XList = new_df
...
trainFilePath = '../data/df/df_train.csv'
testFilePath = '../data/df/df_test.csv'
print("================== 正在加载数据集 ==================")
...
"""
if model == 'rf':
    rnd_clf = RandomForestRegressor(n_estimators=1200, max_leaf_nodes=38, n_jobs=-1)
...
if model == 'ada':
    ...
    y_pred_ada = ada_clf.predict(X_sample)
